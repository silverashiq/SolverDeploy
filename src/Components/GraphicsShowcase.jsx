...
const GraphicsShowcase = () => {
  ...
  const handleFilterChange = (filter) => {
    setLoading(true); // Start loading when filter changes
    setSelectedFilter(filter);
    setTimeout(() => {
      setLoading(false); // Stop loading after images are 'loaded'
    }, 500); // Simulate loading time (you can adjust this based on actual data load time)
  };
  ...
  <div className="flex flex-wrap justify-center space-x-4 gap-y-4 mb-10">
    {["Logo", "Print Media", "Digital Media", "Packaging", "Tshirt"].map((filter) => (
      <button
        key={filter}
        onClick={() => handleFilterChange(filter)} // Use handleFilterChange instead of setSelectedFilter directly
  ...
  {loading ? (
    <div className="flex justify-center items-center h-64 sm:h-40 md:h-60 lg:h-48 bg-gray-200 rounded-2xl">
      <span className="text-xl text-gray-500">Loading...</span> {/* You can replace with a spinner if preferred */}
    </div>
  ) : (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-5 gap-4 mx-0 lg:mx-24">
      {graphicsData[selectedFilter]?.map((item, index) => (
        <div key={index} className="relative group cursor-pointer">
          <div className="w-full  sm:h-40 md:h-60 lg:h-48 bg-gray-200 rounded-2xl overflow-hidden relative">
            <img
              src={item.url}
              alt={item.alt}
              className="w-full h-full object-cover transition duration-300"
              onClick={() => handleModalOpen(item.url)} // Make the image clickable for both desktop and mobile
            />
            ...
            <div className="absolute inset-0 bg-black bg-opacity-0 lg:group-hover:bg-opacity-50 lg:group-hover:backdrop-blur-sm transition duration-300 flex items-center justify-center opacity-0 lg:group-hover:opacity-100">
              <FaSearch
                ...
                onClick={() => handleModalOpen(item.url)} // Image still clickable
              />
            </div>
          </div>
        </div>
